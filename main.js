...
async function processSubscriptionPayment() {
  ...
  const subscriptionPrice = 125; // 125 gourdes
  
  // Vérifier si assez d'argent
  if (currentBalance < subscriptionPrice) {
    alert(`Ou pa gen ase lajan! Ou bezwen ${subscriptionPrice} goud men ou gen sèlman ${currentBalance.toFixed(2)} goud.\n\nAle nan "Depo/Retire" pou ajoute lajan.`);
    return;
  }
  
  // Confirmer le paiement
  const confirmPayment = confirm(`Konfime peman ${subscriptionPrice} goud pou 30 jou abonman?\n\nNouvo balans: ${(currentBalance - subscriptionPrice).toFixed(2)} goud`);
  ...
  try {
    ...
    await db.collection('users').doc(user.uid).update({
      balance: firebase.firestore.FieldValue.increment(-subscriptionPrice)
    });
    ...
    await db.collection('transactions').add({
      ...
      timestamp: firebase.firestore.FieldValue.serverTimestamp(),
      description: 'Abonnement 30 jours'
    });
    ...
    if (success) {
      ...
      alert("Erè pandan aktivasyon abonman. Lajan ou pa dedwi.");
    }
  } catch (error) {
    console.error("Erreur paiement abonnement:", error);
    alert("Erè pandan peman. Eseye ankò.");
  }
}
...
if (subscriptionStatus.isActive) {
  ...
  // Vérifier si le joueur a encore des parties gratuites
  if (userRealData.pati < 10) {
    const partiesRestantes = 10 - userRealData.pati;
    subscriptionHTML = `
      <div class="subscription-status" style="background: linear-gradient(45deg, #22c55e, #16a34a); color: white;">
        ...
        <p>Aprè ${partiesRestantes} pati, w ap gen bezwen yon abonman pou sèlman 125 goud pou 1 mwa</p>
      </div>
    `;
  } else {
    subscriptionHTML = `
    ...
    profilePage.innerHTML = `
      ...
      <main>
        ...
        <div class="money-container">
          <div>
            <div>Lajan ou:</div>
            <div class="money-amount">$${userRealData.balance.toFixed(2)}</div>
          </div>
    ...
    if (transactionsQuery.empty) {
      ...
      transactionsQuery.forEach(doc => {
        ...
        switch(type) {
          ...
          case 'tournament_entry': typeText = 'Antre Tounwa'; break;  // ← AJOUTER
  case 'tournament_win': typeText = 'Genyen Tounwa'; break;   // ← AJOUTER
  case 'tournament_refund': typeText = 'Ranbousman'; break;  
...
/* L’utilisateur doit partager pour débloquer */
function sharePromo() {
  // Ici, le lien de ton site
  const url = "https://chanpyon509.com";
  const text = "Fè lajan ak konpetans ou.";
  ...
  if (navigator.share) {
    ...
    alert("Kopye lyen sa a epi pataje li: " + url);
    unlockGame();
  }
}

/* Débloquer définitivement le bouton Jwe */
function unlockGame() {
  const btn = document.getElementById("playBtn");
  btn.disabled = false;
  btn.classList.add("enabled");
  
  // Cliquez sur jouer → redirection
  btn.onclick = function() {
    window.location.href = 'dino2.html'; // CHANGE CE LIEN
  };
}

  function openParennajModal() {
    document.getElementById("parennajModal").style.display = "flex";
  }

  function closeParennajModal() {
    document.getElementById("parennajModal").style.display = "none";
  }
  
// ===============================
//   WIDGET GAGNANTS RÉCENTS
//   À placer TOUT À LA FIN de ton <script>, juste avant </script>
// ===============================
...
window.addEventListener('load', function() {
  ...
  const widgetHTML = `
    ...
    <div class="winners-panel">
      ...
      <div class="winners-list-container"></div>
    </div>
  `;
  ...
  const btnWidget = document.querySelector('.winners-button');
  const closeBtnWidget = document.querySelector('.close-panel-btn');
  const listContainer = document.querySelector('.winners-list-container');
  ...
  // Fermer le panel
  closeBtnWidget.onclick = function() {
    panel.classList.remove('open');
  };

  // Fonction pour charger les gagnants
  async function loadWinners() {
    listContainer.innerHTML = '<p class="loading-winners">⏳ Chargement des gagnants...</p>';
    ...
    try {
      ...
      if (snapshot.empty) {
        listContainer.innerHTML = '<p class="loading-winners">Aucun tournoi terminé pour le moment.</p>';
        return;
      }
      ...
      for (const docSnapshot of snapshot.docs) {
        ...
        if (t.winnerId) {
          try {
            const userDoc = await db.collection('users').doc(t.winnerId).get();
            if (userDoc.exists) {
        ...
        const completedDate = t.completedAt ? t.completedAt.toDate().toLocaleString('fr-FR') : 'Date inconnue';
        
        item.innerHTML = `
          <div class="winner-title">
            <span class="trophy-icon">👑</span>${username} - ${t.prizePaid || 0} G
          </div>
          <div class="winner-info">
            <strong>Tournoi:</strong> ${t.name || 'Sans nom'}<br>
            <strong>Jeu:</strong> ${t.game || 'Non spécifié'}<br>
            <strong>Participants:</strong> ${t.participantCount || 0}<br>
      ...
      listContainer.innerHTML = '<p class="loading-winners">❌ Erreur de chargement. Réessayez.</p>';
    }
  }
