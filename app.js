...
// ================= CALCUL SOLDE =================
function calculerSolde(choices) {
    let score = 0;

    Object.values(choices).forEach(choice => {
        ['resultat', 'doublechance', 'total', 'btts', 'scoreexact'].forEach(type => {
            if (choice[type] && COTES[type]?.[choice[type]]) {
                score += COTES[type][choice[type]].montant;
            }
        });
    });
...
function updatePanier() {
    ...
    if (validerBtn) validerBtn.disabled = count < 16;
    ...
    if (count === 0) {
        ...
        if (pl) pl.textContent = '0 / 16 match';
    ...
    Object.keys(userChoices).forEach(matchId => {
        ...
        Object.keys(choice).forEach(key => {
            if (key !== 'matchName' && choice[key]) {
                const coteInfo = COTES[key]?.[choice[key]];
                const coteLabel = coteInfo
                    ? (coteInfo.montant < 0
                        ? `<span style="color:#ef4444">${coteInfo.montant.toLocaleString('fr-FR')} Goud</span>`
                        : `<span style="color:#4ade80">+${coteInfo.montant.toLocaleString('fr-FR')} Goud</span>`)
                    : '';
    ...
    const solde = calculerSolde(userChoices);
    ...
    if (progressBar) progressBar.style.width = (count / 16 * 100) + '%';
    if (progressLabel) progressLabel.textContent = count + ' / 16 match';
    if (warning) warning.style.display = solde <= 0 && count > 0 ? 'block' : 'none';

    if (soldeEl) {
        soldeEl.textContent = solde.toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' Goud';
        soldeEl.style.color = solde <= 0 ? '#ef4444' : '#4ade80';
    }

    if (coteTotalEl) {
        coteTotalEl.textContent = count > 0 ? count + ' chwa' : '—';
    }
}
            ...
            <div class="fiche-footer">
                <div class="fiche-info-row">
                    ...
                    <span class="fiche-gains">
                        ${calculerSolde(fiche.choices).toLocaleString('fr-FR', {maximumFractionDigits: 0})} Goud
                    </span>
                </div>
                ${totalChoices > 5 ? `
...
bindClick('validerBtn', async () => {
    const count = Object.keys(userChoices).length;
    if (count < 15) {
        showToast('⚠️ Ou dwe chwazi 15 match anvan valide!');
        return;
    }
...
window.confirmAndSubmitFiche = async function() {
    ...
    if (Object.keys(userChoices).length < 16) {
    showToast('❌ Vérification échouée : 16 choix requis');
    return;
}

const scoreFinal = calculerSolde(userChoices);
if (scoreFinal <= 0) {
    showToast('❌ Skor ou a négatif — Chanje kèk chwa!');
    return;
}
...
const allUserFiches = await window.firebaseApp.db.collection('fiches')
    ...
    if (countWeek >= 3) {
        showToast('⚠️ Ou ka sèlman soumèt 3 fich pa semèn');
        return;
    }
    ...
    try {
        const newFicheRef = await window.firebaseApp.db.collection('fiches').add({
            ...
            choices: userChoices,
            scoreFinal: calculerSolde(userChoices),
            status: 'pending',
...
(function() {
    ...
    overlay.innerHTML = `
        <div class="free-sub-card">
            ...
            <div class="free-sub-features">
                ...
                <div class="free-sub-feature">
                    <span>⚽</span>
                    <span>Chwazi 16 match epi fè prediksyon ou</span>
                </div>
                <div class="free-sub-feature">
                    <span>🏆</span>
    ...
    document.getElementById('freeSubDismiss').onclick = () => overlay.remove();
