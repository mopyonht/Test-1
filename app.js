        ...
        return new Date(matchA.datetime) - new Date(matchB.datetime);
            ...
            toShow.forEach(choice => {
                if (choice.matchName) {
                    ...
                    const matchDateStr = match ? new Date(match.datetime ).toLocaleDateString('fr-FR', {
                        day: 'numeric',
                        month: 'short',
                        hour: '2-digit',
                        minute: '2-digit'
...
async function voterParrain() {
    ...
    if (!userSubscription?.isActive && !userSubscription?.active) {
        showToast('❌ Ou dwe gen yon abònman aktif pou vote');
        return;
    }
    
    const parrainEmail = document.getElementById('parrainEmail').value.trim();
    
    if (!parrainEmail) {
        showToast('⚠️ Antre email parenn ou');
        return;
    }
    
    if (parrainEmail === currentUser.email) {
    ...
    try {
        ...
        if (userDoc.data().referredBy) {
            showToast('⚠️ Ou deja vote pou yon parenn');
            return;
        }
        
        // Chercher le parrain par email
        const parrainSnap = await window.firebaseApp.db.collection('users')
            .where('email', '==', parrainEmail)
            .limit(1)
            .get();
        
        if (parrainSnap.empty) {
            showToast('❌ Email parenn sa pa egziste');
            return;
        }
        
        const parrainId = parrainSnap.docs[0].id;
        const currentReferrals = parrainSnap.docs[0].data().referrals || 0;
        
        // Vérifier limite
        if (currentReferrals >= 5) {
            showToast('⚠️ Parenn sa deja gen 5 parrainages (maksimòm)');
            return;
        }
        
        // Mettre à jour filleul
        await window.firebaseApp.db.collection('users').doc(currentUser.uid).update({
            referredBy: parrainEmail,
            hasVoted: true
        });
        
        // Incrémenter parrain
        await window.firebaseApp.db.collection('users').doc(parrainId).update({
...
function displayParrainageModal() {
    ...
    const minMatchCount = document.getElementById('minMatchCount');
    ...
    window.firebaseApp.db.collection('users').doc(currentUser.uid).get().then(doc => {
        ...
        const minMatches = Math.max(17, 18 - Math.floor(referrals / 5));
        
        if (referralCount) referralCount.textContent = `${referrals}/5`;
        if (minMatchCount) minMatchCount.textContent = minMatches;
