...
window.confirmAndSubmitFiche = async function() {
    ...
    const expiresAt = typeof sub.expiresAt.toDate === 'function' ? sub.expiresAt.toDate() : new Date(sub.expiresAt);
    ...
    // ⭐ LIMITE 3 FICHES PAR SEMAINE - VERSION SIMPLE
    ...
    allUserFiches.forEach(doc => {
        ...
        if (timestamp >= weekAgo) count++;
    });
    
    console.log('Fiches cette semaine:', count);
