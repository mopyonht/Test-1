// ===== SYSTÈME DE TOURNOIS FIREBASE REALTIME - IDANTIK =====
...
function listenToAllTournamentStats() {
  ...
  // Catégories et leurs configurations (NOTER LE SUFFIXE -idantik)
  const configs = [
    { cat: 'test-idantik', players: 2, fees: [25, 50, 100] },
    { cat: 'rapid-idantik', players: 10, fees: [25, 50, 100, 250, 500] },
...
function startCountdown(tournamentId, roomKey) {
  ...
  const interval = setInterval(() => {
    ...
    if (countdown <= 0) {
      ...
      // REDIRECTION VERS IDANTIK (CHANGEMENT ICI)
      window.location.href = 'idantik-tournament.html';
    }
  }, 1000);
}
...
function showVictoryMessage(reward) {
  ...
  overlay.innerHTML = `
    ...
    <button onclick="window.location.href='tournaments-home-idantik.html'" 
