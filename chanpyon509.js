...
async function handleRegister(e) {
    ...
    const phone = document.getElementById('registerPhone').value; // On garde la valeur

    if (password !== confirmPassword) { showToast('❌ Modpas yo pa menm!'); return; }

    // ✅ SUPPRIMÉ : la validation du téléphone et la vérification dans la collection phones
    ...
    try {
        ...
        // Créer le document utilisateur avec le champ phone (optionnel)
        await window.firebaseApp.db.collection('users').doc(user.uid).set({
            ...
            referrals: 0,
            referralPoints: 0,
            phone: phone || '' // On ajoute le téléphone même sans validation
        });

        // ✅ SUPPRIMÉ : l'écriture dans la collection 'phones'
...
// ================= MODAL LIEN YOUTUBE (UNE SEULE FOIS PAR SESSION) =================
(function initYoutubeLinkModal() {
    // Vérifier si déjà affiché dans cette session
    if (sessionStorage.getItem('c509_youtube_link_seen')) return;

    let introFinished = false;

    function createModal() {
        if (document.getElementById('youtubeLinkModal')) return;

        const modal = document.createElement('div');
        modal.className = 'modal-overlay';
        modal.id = 'youtubeLinkModal';
        modal.innerHTML = `
            <div class="auth-modal" style="max-width:400px; text-align:center;">
                <button class="close-modal" onclick="closeYoutubeLinkModal()">✕</button>
                <div style="font-size:60px; margin:10px 0 5px; color:#ff0000;">▶️</div>
                <h2 class="auth-title" style="margin-bottom:10px;">Gade videyo sou YouTube</h2>
                <p style="color:var(--text-secondary); margin-bottom:20px; font-size:14px;">
                    Kòman Anti-paryaj Tounwa fonksyone ?<br>
                    Klike sou bouton an pou w gade eksplikasyon an.
                </p>
                <a href="https://youtu.be/1zU-N0NbvCM" 
                   target="_blank" 
                   rel="noopener noreferrer"
                   class="btn-auth" 
                   style="display:flex; align-items:center; justify-content:center; gap:10px; text-decoration:none; margin-bottom:10px;"
                   onclick="closeYoutubeLinkModal()">
                    <span>📺</span>
                    <span>Gade sou YouTube</span>
                    <span>🔴</span>
                </a>
                <button onclick="closeYoutubeLinkModal()" 
                        style="background:none; border:none; color:var(--text-dim); font-size:13px; cursor:pointer; padding:8px; width:100%;">
                    Pa kounye a
                </button>
            </div>
        `;
        ...
        // Fermeture propre
        window.closeYoutubeLinkModal = function() {
            const m = document.getElementById('youtubeLinkModal');
            if (m) {
                m.classList.remove('show');
                document.body.style.overflow = 'auto';
                sessionStorage.setItem('c509_youtube_link_seen', 'true');
            }
        };
    ...
    function showModalAfterIntro() {
        createModal();
        const modal = document.getElementById('youtubeLinkModal');
        if (modal) {
            modal.classList.add('show');
            document.body.style.overflow = 'hidden';
        }
    ...
    function checkIntroEnd() {
        // Observer la disparition de l'intro
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                mutation.removedNodes.forEach(function(node) {
                    if (node.id === 'ap-overlay') {
                        introFinished = true;
                        observer.disconnect();
                        showModalAfterIntro();
                    }
        ...
        // Si l'intro n'existe pas, attendre 5 secondes
        setTimeout(function() {
            if (!document.getElementById('ap-overlay') && !introFinished) {
                introFinished = true;
                observer.disconnect();
                showModalAfterIntro();
            }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', checkIntroEnd);
    } else {
        checkIntroEnd();
    }
})();
