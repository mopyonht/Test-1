...
// ================= MODAL YOUTUBE (APRÈS 10 SECONDES) =================
(function initYoutubeModal() {
    // Variables pour contrôler l'affichage
    let modalShown = false;
    let introFinished = false;
    
    // Créer le modal une seule fois (caché au départ)
    function createYoutubeModal() {
        // Vérifier si le modal existe déjà
        if (document.getElementById('youtubeChannelModal')) return;
        
        const modal = document.createElement('div');
        modal.className = 'modal-overlay';
        modal.id = 'youtubeChannelModal';
        modal.innerHTML = `
            <div class="auth-modal" style="max-width:400px; text-align:center;">
                <button class="close-modal" onclick="closeYoutubeModal()">✕</button>
                <div style="font-size:48px; margin:10px 0 5px;">🎥</div>
                <h2 class="auth-title" style="margin-bottom:5px;">Ou poko konprann sistèm Anti-Paryaj la ?</h2>
                <p style="color:var(--text-secondary); font-size:14px; margin-bottom:20px; line-height:1.5;">
                    Ou vle aprann kòman pou w fè 10,000 goud oswa 50,000 goud pandan wap jwe sèlman sou 5 match ?<br>
                    Ale gade videyo eksplikasyon nou yo !
                </p>
                <a href="https://youtube.com/@antiparyaj?si=iTl9Fva4dHopSvUY" 
                   target="_blank" 
                   ...
                   onclick="closeYoutubeModal()">
                    <span style="display:flex; align-items:center; justify-content:center; gap:10px;">
                        <span>🔴</span>
                        <span>Gade videyo sou Youtube</span>
                        <span>▶️</span>
                    </span>
                </a>
                ...
                <div style="margin-top:15px; font-size:11px; color:var(--text-dim); border-top:1px solid var(--border); padding-top:12px;">
                    <span>⭐ Anti-Paryaj : nou ranmase l, ou touche l⭐</span>
                </div>
            </div>
        `;
    ...
    window.closeYoutubeModal = function() {
        const modal = document.getElementById('youtubeChannelModal');
        if (modal) {
            modal.classList.remove('show');
            document.body.style.overflow = 'auto';
        }
    ...
    window.openYoutubeModal = function() {
        createYoutubeModal(); // S'assurer qu'il existe
        const modal = document.getElementById('youtubeChannelModal');
        if (modal) {
            modal.classList.add('show');
            document.body.style.overflow = 'hidden';
    ...
    // Détecter la fin de l'intro
    function checkIntroEnd() {
        // Méthode 1: Observer la disparition de l'overlay d'intro
            ...
            mutations.forEach(function(mutation) {
                mutation.removedNodes.forEach(function(node) {
                    if (node.id === 'ap-overlay') {
                        introFinished = true;
                        observer.disconnect();
                        // Démarrer le timer si l'intro vient de finir
                        startTimerAfterIntro();
                    }
                });
            });
        });
        ...
        // Méthode 2: Si l'intro n'existe pas ou a déjà disparu
        setTimeout(function() {
    ...
    function startTimerAfterIntro() {
        // Si le modal a déjà été montré, ne rien faire
        if (modalShown) return;
        
        // Attendre 10 secondes après l'intro
        setTimeout(function() {
            // Vérifier que l'utilisateur est toujours sur la page
            if (!modalShown && document.visibilityState === 'visible') {
                openYoutubeModal();
            }
        }, 10000); // 10 secondes
    }
    ...
    // Initialisation
    function init() {
        createYoutubeModal(); // Créer le modal en cache
        checkIntroEnd(); // Surveiller la fin de l'intro
    }
